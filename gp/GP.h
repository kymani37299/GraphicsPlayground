...
namespace GP
{
	...
	// Game state
	GP_DLL void Init(HINSTANCE hInstance, unsigned int windowWidth, unsigned int windowHeight, const std::string& windowTitle = "", unsigned int fps = 60, bool vsync = false);
	GP_DLL void Run();
	GP_DLL void Reset();
	GP_DLL void Deinit();
